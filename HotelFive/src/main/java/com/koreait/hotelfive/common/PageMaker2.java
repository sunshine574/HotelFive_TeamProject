...
public class PageMaker2 {
	...
	public static String getPageView(String path, int page, int recordPerPage, int totalRecord, String gName) {
		...
		if ( beginPageOfBlock < pagePerBlock ) {
			...
			sb.append("<a href='" + path + "?page=" + (beginPageOfBlock - 1) + "&gName=" + gName +"'>◀</a>&nbsp;&nbsp;");
		}
		...
		for ( int p = beginPageOfBlock; p <= endPageOfBlock; p++ ) {
			if ( p == page ) {
				...
				sb.append("<a href='" + path + "?page=" + p + "&gName=" + gName +"'>" + p + "</a>&nbsp;&nbsp;");
			}
		}
		...
		if ( endPageOfBlock == totalPage ) {
			...
			sb.append("<a href='" + path + "?page=" + (endPageOfBlock + 1) + "&gName=" + gName +"'>▶</a>");
		}
