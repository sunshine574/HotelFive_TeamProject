...
public class NoticeBoardDownloadCommand implements Command {
	...
	public void execute(SqlSession sqlSession, Model model) {
		...
		String realPath = request.getSession().getServletContext().getRealPath("/resources/assets/notice_files");
		
		// 2. 다운로드 받을 파일 이름은 request 에 nFilename 으로 저장되어 있다.
		String nFilename = request.getParameter("nFilename");
		
		// 업로드 할 때 파일 : car.jpg
		// nFilename         : car_111111111111.jpg
		
		// 3. 업로드 할 때의 파일 이름으로 다운로드 할 파일 이름을 결정해 둔다.
		String extName = nFilename.substring(nFilename.lastIndexOf(".") + 1);  // 확장자만 빼 두기
		String downloadFilename = nFilename.substring(0, nFilename.lastIndexOf("_")) +
								  "." + extName;
		
		// 4. 다운로드 할 파일을 연결한다.
		File file = new File(realPath, nFilename);  // (경로, 파일명)
